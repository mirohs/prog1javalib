...
import prog1.graphics.Graphics;
    ...
    private static class Node { // node of red-black tree, inner class
        ...
        private boolean contains(int x) {
            ...
            if (x < value && left != null) {
                return left.contains(x); // it cannot be in the right subtree
            }
            if (x > value && right != null) {
                return right.contains(x); // it cannot be in the left subtree
            }
            return false;
        }
        ...
        public Image toImage(Graphics g) {
            if (frame == null || empty == null) {
                frame = g.overlay(
                        g.rectangle(40, 1, "transparent"),
                        g.circle(20, "white", pen("black")));
                empty = g.overlay(
                        g.rectangle(40, 1, "transparent"),
                        g.circle(10, "transparent"));
            }
            Image n = g.overlay(
                    g.text("" + value, 14, "black"),
                    frame);
            Image result = g.above(n,
                    g.rotate(0, g.beside("top",
                                    (left != null) ? left.toImage(g) : empty,
                                    (right != null) ? right.toImage(g) : empty)));
            return result;
        }
