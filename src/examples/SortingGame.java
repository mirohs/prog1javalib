...
public class SortingGame extends javafx.application.Application {
	...
	private Image onDraw() {
		Image result = rectangle(WIDTH, HEIGHT, pen("black"));
		for (int i = 0; i < icons.length; i++) {
			Image icon = icons[i];
			Point p = positions[i];
			result = underlay("left", "top", p.x, p.y, result, icon);
		}
		return result;
	}
	
	private static Image makeIcon(int codePoint) {
		// Unicode Emoji:
		...
		return overlay(text(emoji, "OpenSansEmoji", ICON_SIZE - 15, "black"), 
				square(ICON_SIZE, "floralwhite", pen("gray", 2)));
	}

	@Override
	public void start(javafx.stage.Stage stage) {
		ApplicationBase.start("Sorting Game", WIDTH, HEIGHT, stage, this::onDraw);
	}
